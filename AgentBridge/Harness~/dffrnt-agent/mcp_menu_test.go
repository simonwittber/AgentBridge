...
func TestMCP_MenuItem_Valid(t *testing.T) {
	...
	p := c.invokeCmd(t, "menu_item", map[string]any{"path": "Window/General/Inspector"})
	if p["status"] != "ok" {
...
func TestMCP_MenuItem_Invalid(t *testing.T) {
	...
	p := c.invokeCmd(t, "menu_item", map[string]any{"path": "DoesNotExist/FakeMenu/Item"})
	if p["status"] != "error" {
...
func TestMCP_MenuItem_MissingPath(t *testing.T) {
	...
	p := c.invokeCmd(t, "menu_item", map[string]any{})
	if p["status"] != "error" {
